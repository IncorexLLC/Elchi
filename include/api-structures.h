...
struct Dictionary_s {
    Dict_node *     root;
    wchar_t *          name;
    /* wchar_t *          post_process_filename; */  /* was not being used *DS* */
    ...
    FILE *          fp;
    wchar_t         token[MAX_TOKEN_LENGTH];
    int             is_special;
    int             already_got_it;
    int             line_number;
};
...
struct Sentence_s {
    ...
    wchar_t * is_conjunction;      /* TRUE if conjunction, as defined by dictionary */
    wchar_t** deletable;           /* deletable regions in a sentence with conjunction */
    wchar_t** effective_dist;     
    ...
    wchar_t  q_pruned_rules;       /* don't prune rules more than once in p.p. */
};
...
struct Sublinkage_s {
    ...
    wchar_t *    violation;          /* Name of violation, if any */
    PP_data   pp_data;
};
...
struct Linkage_s {
    ...
    wchar_t *        * word;       /* array of word spellings */
