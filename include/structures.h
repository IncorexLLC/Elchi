...
#if defined(WIN32)
#define strncasecmp(a,b,s) _wcsnicmp((a),(b),(s))
#endif
...
#define LEFT_WALL_DISPLAY  (L"LEFT-WALL")   /* the string to use to show the wall */
#define LEFT_WALL_SUPPRESS (L"Wd") /* If this connector is used on the wall, */
				  /* then suppress the display of the wall  */
                                  /* bogus name to prevent ever suppressing */
#define RIGHT_WALL_DISPLAY  (L"RIGHT-WALL")   /* the string to use to show the wall */
#define RIGHT_WALL_SUPPRESS (L"RW") /* If this connector is used on the wall, */

/* The following define the names of the special strings in the dictionary. */
#define LEFT_WALL_WORD   (L"LEFT-WALL")
#define RIGHT_WALL_WORD  (L"RIGHT-WALL")
#define POSTPROCESS_WORD (L"POSTPROCESS")
#define ANDABLE_CONNECTORS_WORD (L"ANDABLE-CONNECTORS")
#define UNLIMITED_CONNECTORS_WORD (L"UNLIMITED-CONNECTORS")
...
#define PROPER_WORD      (L"CAPITALIZED-WORDS")
#define PL_PROPER_WORD      (L"PL-CAPITALIZED-WORDS")
#define HYPHENATED_WORD  (L"HYPHENATED-WORDS")
#define NUMBER_WORD      (L"NUMBERS")
#define ING_WORD         (L"ING-WORDS")
#define S_WORD           (L"S-WORDS")
#define ED_WORD          (L"ED-WORDS")
#define LY_WORD          (L"LY-WORDS")
#define UNKNOWN_WORD     (L"UNKNOWN-WORD")
...
struct Connector_struct{
  ...
  /*    wchar_t my_word; */  /* not used now */
                  /* The word that this connector arises from */
    wchar_t priority;/* one of the three priorities above */
    wchar_t multi;   /* TRUE if this is a multi-connector */
    Connector * next;
    wchar_t * string;
};
...
struct Link_s {
    int l, r;
    Connector * lc, * rc;
    wchar_t * name;              /* spelling of full link name */
};
...
struct X_node_struct {
    wchar_t * string;  /* the word itself */
    Exp * exp;
    X_node *next;
};
...
struct Exp_struct {
    wchar_t type;            /* one of three types, see below */
    wchar_t cost;   /* The cost of using this expression.
			     Only used for non-terminals */
    wchar_t dir;   /* '-' means to the left, '+' means to right (for connector) */
    wchar_t multi; /* TRUE if a multi-connector (for connector)  */
    union {
		E_list * l;       /* only needed for non-terminals */
		wchar_t * string;    /* only needed if it's a connector */
    }u;
    Exp * next;
};
...
struct Word_file_struct {
    wchar_t file[MAX_PATH_NAME+1];   /* the file name */
...
struct Andlist_struct {
    ...
    int conjunction;
    int num_elements;
    wint_t element[MAX_SENTENCE];
    int num_outside_words;
    wint_t outside_word[MAX_SENTENCE];
    int cost;
};
...
struct Linkage_info_struct {
    ...
    wchar_t fat;
    wchar_t canonical;
    wchar_t improper_fat_linkage;
    wchar_t inconsistent_domains;
...
struct List_o_links_struct{
    ...
    wint_t word;       /* the word at the other end of this link */
...
typedef struct {
    ...
    wchar_t ** table;  /* the table itself */
} String_set;
...
typedef struct PPLexTable_s
{
  ...
  wchar_t *labels[PP_LEXER_MAX_LABELS];    /* array of labels (NULL-terminated) */
...
typedef struct pp_rule_s
{
  ...
  wchar_t *selector;       /* name of link to which rule applies      */
  ...
  wchar_t  **link_array;   /* array containing the spelled-out names  */
  wchar_t  *msg;           /* explanation (NULL=end sentinel in array)*/
} pp_rule;

typedef struct pp_knowledge_s
{
  PPLexTable *lt; /* Internal rep'n of sets of strings from knowledge file */
  wchar_t *path;     /* Name of file we loaded from */
