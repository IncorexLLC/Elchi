...
*/
	...
	if ((wcscmp(sent->word[w].string, L",")==0) && sent->is_conjunction[w+1]) {
	    sent->word[w].d = catenate_disjuncts(special_disjunct(COMMA_LABEL,L'+',L"", L","), sent->word[w].d);
...
static void construct_either(Sentence sent) {
    int w;
    if (!sentence_contains(sent, L"either")) return;
    for (w=0; w<sent->length; w++) {
	if (wcscmp(sent->word[w].string, L"either") != 0) continue;
	sent->word[w].d = catenate_disjuncts(
                   special_disjunct(EITHER_LABEL,L'+',L"", L"either"),
                   sent->word[w].d);
    }

    for (w=0; w<sent->length; w++) {
	if (wcscmp(sent->word[w].string, L"or") != 0) continue;
...
static void construct_notonlybut(Sentence sent) {
    int w;
    Disjunct *d;
    if (!sentence_contains(sent, L"not")) {
	return;
    }
    for (w=0; w<sent->length; w++) {
	if (wcscmp(sent->word[w].string, L"not") != 0) continue;
	sent->word[w].d = catenate_disjuncts(
	     special_disjunct(NOT_LABEL,L'+',L"", L"not"),
	     sent->word[w].d);
	if (w<sent->length-1 &&  wcscmp(sent->word[w+1].string, L"only")==0) {
	    sent->word[w+1].d = catenate_disjuncts(
                          special_disjunct(NOTONLY_LABEL, L'-',L"",L"only"),
                          sent->word[w+1].d);
	    d = special_disjunct(NOTONLY_LABEL, L'+', L"",L"not");
	    d = add_one_connector(NOT_LABEL,L'+',L"", d);
     ...
     */
    for (w=0; w<sent->length; w++) {
	if (wcscmp(sent->word[w].string, L"but") != 0) continue;
	sent->word[w].d = glom_aux_connector
...
static void construct_both(Sentence sent) {
    int w;
    if (!sentence_contains(sent, L"both")) return;
    for (w=0; w<sent->length; w++) {
	if (wcscmp(sent->word[w].string, L"both") != 0) continue;
	sent->word[w].d = catenate_disjuncts(
                   special_disjunct(BOTH_LABEL,L'+',L"", L"both"),
                   sent->word[w].d);
    }

    for (w=0; w<sent->length; w++) {
	if (wcscmp(sent->word[w].string, L"and") != 0) continue;
