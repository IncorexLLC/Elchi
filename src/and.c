...
void grow_LT(Sentence sent) {
	...
	wprintf_s(L"Ran out of space reallocing the label table\n");
	exit(1);
    }
}
...
void free_HT(Sentence sent) {
	...
	for (la=sent->and_data.hash_table[i]; la != NULL; la = la1) {
	    la1 = la->next;
	    xfree((wchar_t *)la, sizeof(Label_node));
	}
	sent->and_data.hash_table[i] = NULL;
    }
}
...
void free_LT(Sentence sent) {
    ...
    xfree((wchar_t *) sent->and_data.label_table, 
	  sent->and_data.LT_bound * sizeof(Disjunct*));
...
*/
	...
	if (!match_in_connector_set(sent->dict->andable_connector_set, c, L'+')) return FALSE;
    }
...
wchar_t * intersect_strings(Sentence sent, wchar_t * s, wchar_t * t) {
...
*/
    int len, i, j, d;
    wchar_t * u, *u0, *s0;
    if (wcscmp(s,t)==0) return s;  /* would work without this */
    i = wcslen(s);
    j = wcslen(t);
    if (j > i) {
	u = s; s = t; t = u;
    ...
    u0 = u = (wchar_t *) xalloc(sizeof(wchar_t)*(len+1));
    d = 0;
    s0 = s;
    while (*t != L'\0') {
	if ((*s == *t) || (*t == L'*')) {
    ...
    if (d==0) {
	xfree(u0, sizeof(wchar_t)*(len+1));
	return s0;
    } else {
	wcscpy(u, s);   /* get the remainder of s */
...
wchar_t * stick_in_one_connector(wchar_t *s, Connector *c, int len) {
...
*/
    wchar_t * t;
    for (t = c->string; iswupper((wint_t)*t); ++t)
      ;
    while (*t != L'\0') {
	*s++ = *t++;
	len--;
	...
	*s++ = L'*';
    ...
    if (c->multi) *s++ = L'*'; else *s++ = L'^';  /* check this sometime */
    *s = L'\0';
    return s;
}
...
*/   
	...
	i=0;
	os = s = (wchar_t *) xalloc(sizeof(wchar_t)*(tot_len+1));
	for (c=d->left; c != NULL; c = c->next) {
	...
	xfree(os, sizeof(wchar_t)*(tot_len+1));
	d->string = s;
    }
    xfree((wchar_t *)lengths, N_connectors*sizeof(int));
}
...
void print_AND_statistics(Sentence sent) {
    wprintf_s(L"Number of disjunct types (labels): %d\n", sent->and_data.LT_size);
    wprintf_s(L"Number of disjuncts in the table: %d\n", STAT_N_disjuncts);
    if (sent->and_data.LT_size != 0) {
      wprintf_s(L"average list length: %f\n",
	     (float)STAT_N_disjuncts/sent->and_data.LT_size);
    }
    wprintf_s(L"Number of equality tests: %d\n", STAT_calls_to_equality_test);
}
...
*/
    ...
    assert(lp!=NULL, L"A disjunct I inserted was not there. (1)");
/*
  I don't know what these lines were for.  I replaced them by
  the above assertion.
    if (lp == NULL) {
	wprintf_s("error: A disjunct I inserted was not there\n");
...
*/
    ...
    assert(d1!=NULL, L"A disjunct I inserted was not there. (2)");
...
Disjunct * build_AND_disjunct_list(Sentence sent, wchar_t * s) {
 ...
 /* the string is either "and", or "or", or "nor" at the moment */
    ...
    */
	...
	for (d1 = d_list; d1!=NULL; d1=d1->next) {
		...
		if ((c1->string[0] == L'S') && (c1->string[1]==L'I') &&
		    ((c1->string[2]==L'^') ||
		     (c1->string[2]==L's') ||
		     (c1->string[2]==L'p') ||
		     (c1->string[2]==L'\0'))) {
...
*/
    else if ((wcscmp(s,L"nor")==0) || (wcscmp(s,L"or")==0)) {
	for (d1 = d_list; d1!=NULL; d1=d1->next) {
	    for (c1=d1->right; c1!=NULL; c1=c1->next) {
		if ((c1->string[0] == L'S') &&
		    ((c1->string[1]==L'^') ||
...
static wchar_t has_fat_down[MAX_SENTENCE];  /* TRUE if this word has a fat down link
				     FALSE otherise */
...
void free_image_array(Parse_info * pi) {
	...
	for (in=image_array[w]; in!=NULL; in=inx) {
	    inx = in->next;
	    xfree((wchar_t *)in, sizeof(Image_node));
	}
    }
}	    
...
int strictly_smaller(wchar_t * s, wchar_t * t) {
...
*/
    ...
    assert(! ((*s!=L'\0') || (*t!=L'\0')), L"s and t should be the same length!");
    return (strictness > 0);  
}
...
/*	    if ((cx->string != cy->string) || */
	    if ((wcscmp(cx->string, cy->string) != 0) ||
...
/*	    if ((cx->string != cy->string) || */
    ...
    assert(d!=NULL, L"Never found subdisjunct");
    return d;
}
...
*/
	...
	assert(d_c != NULL, L"Should have found the down link.");
	...
	for (dis=sent->and_data.label_table[d_label]; dis!=NULL; dis=dis->next) {
		...
		if (place == 0) {
		    ...
		    assert(upcon != NULL, L"Should have found an up link");
...
*/
	...
	for (link=0; link<pi->N_links; ++link) {
		...
		wprintf_s(L"word = %d\n", word);
		wprintf_s(L"fat link: [%d, %d]\n", 
		       pi->link_array[link].l, pi->link_array[link].r);
		wprintf_s(L"thin link: [%d, %d]\n", 
		       sublinkage->link[link]->l, sublinkage->link[link]->r);
		assert(FALSE, L"There should be a fat UP link here");
	    }
	    ...
	    assert(dis!=NULL, L"Should have found this connector string");
