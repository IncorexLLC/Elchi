...
int evaluate_disjunct(Disjunct * d, Alink * alink, int currentword) {
	...
	if (wcscmp(al->connector, L"XR")==0) cok = 1;
	if (cok==0) dok=0;
      }
    }	    
	...
	if (wcscmp(al->connector, L"XL")==0) cok = 1;
	if (cok==0) dok=0;
      }
    }	
    ...
    if(dok==1) {
      ...
      if(localv == 2) {
	wprintf_s(L"  For word %s, disjunct found: ", tword[currentword].gstring); 	  
	wprintf_s(L"L: ");
	c = d->left;
	for(; c!=NULL; c=c->next) {
	  wprintf_s(L"%s ", c->string);	
	}
	wprintf_s(L"; R: ");
	c = d->right;
	for(; c!=NULL; c=c->next) {
    ...
    if(dok==0){
      /* wprintf_s("No disjunct found for '%s'\n", ds); */
      return 0; 
    }
}
...
void insert_conjunction_commas(Alinkset * alinkset) {
	...
	if(previous>-1) {
	  ...
	  if(localv == 2) wprintf_s(L"Inserting a comma between %d and %d at position %6.6f\n", previous, lowest, word_position[numwords]);
	  numwords++;
	}
	previous = lowest;
      }
...
int print_words_in_order(Alink * alink) {
	...
	if(wcscmp(tword[w].gstring, L"LEFT-WALL")==0 || wcscmp(tword[w].gstring, L"RIGHT-WALL")==0) break;

	/* If the current word is a comma, and it's the last word (or the next word is a period or the right-wall), 
	   don't print it. */
	if(wcscmp(tword[w].gstring, L",")==0) {
	  if(p==numwords-1) break;
	  if(wcscmp(tword[final_order[p+1]].gstring, L"RIGHT-WALL")==0 || wcscmp(tword[final_order[p+1]].gstring, L".")==0 ||
	     wcscmp(tword[final_order[p+1]].gstring, L"?")==0 || wcscmp(tword[final_order[p+1]].gstring, L"!")==0) break;
	}

	if(tword[final_order[p-1]].particle==1 & wcscmp(tword[final_order[p]].gstring, L"zu")==0) tword[final_order[p]].particle=1;
	contraction = 1;
	if(wcscmp(tword[w].gstring, L"zu")==0 && wcscmp(tword[final_order[p+1]].gstring, L"dem.d")==0) wcscpy(word, L"zum");
	else if(wcscmp(tword[w].gstring, L"zu")==0 && wcscmp(tword[final_order[p+1]].gstring, L"der.d")==0) wcscpy(word, L"zur");
	else if(wcscmp(tword[w].gstring, L"in")==0 && wcscmp(tword[final_order[p+1]].gstring, L"dem.d")==0) wcscpy(word, L"im");
	else if(wcscmp(tword[w].gstring, L"an")==0 && wcscmp(tword[final_order[p+1]].gstring, L"dem.d")==0) wcscpy(word, L"am");
	else if(wcscmp(tword[w].gstring, L"ins")==0 && wcscmp(tword[final_order[p+1]].gstring, L"das.d")==0) wcscpy(word, L"ins");

	else {	

	  if(wcscmp(tword[w].gstring, L"NAME")==0) wcscpy(word, tword[w].estring);
	...
	if(word[0]!=L',' && word[0]!=L'.' && word[0]!=L'?' && word[0]!=L'!' && wcscmp(word, L"s")!=0 &&
	   tword[final_order[p-1]].particle!=1 && tword[final_order[p]].particle!=2) wprintf_s(L" "); 
    ...
    if(localv == 3) {
      wprintf_s(L"!top GERMAN TRANSLATION: ");
      for(p=0; p<numwords; p++) {
        if(final_word[p]==NULL) continue;
	if(wcscmp(final_word[p], L",")!=0 && wcscmp(final_word[p], L".")!=0 && wcscmp(final_word[p], L"?")!=0 && wcscmp(final_word[p], L"!")!=0 && wcscmp(final_word[p], L"s")!=0 && tword[final_order[p-1]].particle!=1 && tword[final_order[p]].particle!=2) wprintf_s(L" ");
        wprintf_s(L"%s", final_word[p]);
      }
      wprintf_s(L"\n");
    }
