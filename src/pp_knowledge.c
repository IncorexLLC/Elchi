...
static void check_domain_is_legal(wchar_t *p) 
{
  if (wcslen(p)>1)
    error(L"post_process: Domain (%s) must be a single character", p);
}
...
static void read_starting_link_table(pp_knowledge *k)
{
  ...
  wchar_t *p;
  const wchar_t label[] = L"STARTING_LINK_TYPE_TABLE";
  int i, n_tokens;
  if (!pp_lexer_set_label(k->lt, label))
    error(L"post_process: Couldn't find starting link table %s",label);
  n_tokens = pp_lexer_count_tokens_of_label(k->lt); 
  if (n_tokens %2) 
    error(L"post_process: Link table must have format [<link> <domain name>]+");
...
static pp_linkset *read_link_set(pp_knowledge *k,
				 const wchar_t *label, String_set *ss)
{
  ...
  if (!pp_lexer_set_label(k->lt, label)) {
    if (verbosity>0)
      wprintf_s(L"PP warning: Link set %s not defined: assuming empty.\n",label);
    n_strings = 0;
  }
...
static void read_link_sets(pp_knowledge *k)
{
  ...
  k->domain_starter_links     =read_link_set(k,L"DOMAIN_STARTER_LINKS",ss);
  k->urfl_domain_starter_links=read_link_set(k,L"URFL_DOMAIN_STARTER_LINKS",ss);
  k->domain_contains_links    =read_link_set(k,L"DOMAIN_CONTAINS_LINKS",ss);
  k->ignore_these_links       =read_link_set(k,L"IGNORE_THESE_LINKS",ss);
  k->restricted_links         =read_link_set(k,L"RESTRICTED_LINKS",ss);
  k->must_form_a_cycle_links  =read_link_set(k,L"MUST_FORM_A_CYCLE_LINKS",ss); 
  k->urfl_only_domain_starter_links=
      read_link_set(k,L"URFL_ONLY_DOMAIN_STARTER_LINKS",ss);
  k->left_domain_starter_links=read_link_set(k,L"LEFT_DOMAIN_STARTER_LINKS",ss);
}
...
static void read_connected_rule(pp_knowledge *k, const wchar_t *label)
{
  ...
  if (!pp_lexer_set_label(k->lt, label))
    {
      ...
      if (verbosity>0) wprintf_s(L"PP warning: Not using 'link is connected' rule\n");
      return;
    }
  if (pp_lexer_count_tokens_of_label(k->lt)>1) 
    error(L"post_process: Invalid syntax in %s", label);
  k->connected_rules[0].msg =
    string_set_add(pp_lexer_get_next_token_of_label(k->lt), k->string_set);
}


static void read_form_a_cycle_rules(pp_knowledge *k, const wchar_t *label)
{
  ...
  if (!pp_lexer_set_label(k->lt, label)) {
      ...
      if (verbosity>0)
	wprintf_s(L"PP warning: Not using any 'form a cycle' rules\n");
  }
  else {
  ...
  for (r=0; r<k->n_form_a_cycle_rules; r++)
    {
      ...
      if (n_tokens <= 0) error(L"syntax error in knowledge file");
	...
	pp_linkset_add(lsHandle,string_set_add(tokens[i], k->string_set));
      ...
      tokens = pp_lexer_get_next_group_of_tokens_of_label(k->lt, &n_tokens);
      if (n_tokens>1)
	error(L"post_process: Invalid syntax (rule %i of %s)",r+1,label);
...
static void read_bounded_rules(pp_knowledge *k, const wchar_t *label)
{
  wchar_t **tokens;
  int n_commas, n_tokens, r;
  if (!pp_lexer_set_label(k->lt, label)) {
      k->n_bounded_rules = 0;
      if (verbosity>0) wprintf_s(L"PP warning: Not using any 'bounded' rules\n");
  }
  else {
    n_commas = pp_lexer_count_commas_of_label(k->lt); 
...
static void read_contains_rules(pp_knowledge *k, wchar_t *label, pp_rule **rules, 
				int *nRules)
{
  ...
  if (!pp_lexer_set_label(k->lt, label)) {
      *nRules = 0;
      if (verbosity>0) wprintf_s(L"PP warning: Not using any %s rules\n", label);
  }
  else {
    n_commas = pp_lexer_count_commas_of_label(k->lt);
    *nRules = (n_commas + 1)/3;
  ...
  for (r=0; r<*nRules; r++)
    {
      ...
      tokens = pp_lexer_get_next_group_of_tokens_of_label(k->lt, &n_tokens);
      if (n_tokens>1) 
	error(L"post_process: Invalid syntax in %s (rule %i)",label,r+1);
      ...
      (*rules)[r].link_array = (wchar_t **) xalloc((1+n_tokens)*sizeof(wchar_t*));
      for (i=0; i<n_tokens; i++) 
	{
...
{
  read_form_a_cycle_rules(k, L"FORM_A_CYCLE_RULES");
  read_connected_rule(k, L"CONNECTED_RULES");
  read_bounded_rules(k,  L"BOUNDED_RULES");
  read_contains_rules(k, L"CONTAINS_ONE_RULES" ,
		      &(k->contains_one_rules), &(k->n_contains_one_rules)); 
  read_contains_rules(k, L"CONTAINS_NONE_RULES",
...
static void free_rules(pp_knowledge *k)
{
  ...
  for (r=0; k->contains_one_rules[r].msg!=0; r++) {
    ...
    xfree((void*) rule->link_array, (1+rule->link_set_size)*sizeof(wchar_t*));
...
pp_knowledge *pp_knowledge_open(wchar_t *dictname, wchar_t *path)
{
  /* read knowledge from disk into pp_knowledge */
  FILE *f=dictopen(dictname, path, L"r");
  pp_knowledge *k = (pp_knowledge *) xalloc (sizeof(pp_knowledge));
  if (!f) error(L"Couldn't find post-process knowledge file %s", path);
