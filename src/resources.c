...
void print_time(Parse_Options opts, wchar_t * s) {
...
void resources_print_time(int verbosity, Resources r, wchar_t * s) {
/* print out the cpu ticks since this was last called */
    double new_t;
    new_t = current_usage_time();
    if (verbosity > 1) {
	wprintf_s(L"++++");
	left_print_string(stdout, s, L"                                           ");
	wprintf_s(L"%7.2f seconds\n", new_t - r->when_last_called);
    }
...
/* print out the cpu ticks since this was last called */
    ...
    if (verbosity > 0) {
        wprintf_s(L"++++");
	left_print_string(stdout, L"Time",
			  L"                                           ");
	wprintf_s(L"%7.2f seconds (%.2f total)\n", 
...
void resources_print_total_space(int verbosity, Resources r) {
    if (verbosity > 1) {
       wprintf_s(L"++++");
      left_print_string(stdout, L"Total space",
			L"                                            ");
      wprintf_s(L"%d bytes (%d max)\n", space_in_use, max_space_in_use);
    }
}