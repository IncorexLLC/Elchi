...
static int string_in_list(wchar_t * s, wchar_t * a[]) 
{
...
static int find_domain_name(Postprocessor *pp, wchar_t *link) 
{
...
static void setup_domain_array(Postprocessor *pp,
			       int n, wchar_t *string, int start_link) 
{
...
static void build_domains(Postprocessor *pp, Sublinkage *sublinkage)
{
  ...
  for (link = 0; link<sublinkage->num_links; link++) {
	...
	pp->pp_data.N_domains++;
        assert(pp->pp_data.N_domains<PP_MAX_DOMAINS, L"raise value of PP_MAX_DOMAINS");
      } 
    else {
      if (pp_linkset_match(pp->knowledge->urfl_domain_starter_links,s)) 
	{
	  ...
	  bad_depth_first_search(pp,sublinkage,sublinkage->link[link]->r,
				 sublinkage->link[link]->l,link);
	  pp->pp_data.N_domains++;
	  assert(pp->pp_data.N_domains<PP_MAX_DOMAINS,L"raise PP_MAX_DOMAINS value");
	}
      else 
  ...
  for (d=0; d<pp->pp_data.N_domains; d++) {
    ...
    if (i==-1) 
       error(L"\tpost_process: Need an entry for %s in LINK_TYPE_TABLE",
...
static void build_domain_forest(Postprocessor *pp, Sublinkage *sublinkage)
{
  ...
  for (d=0; d < pp->pp_data.N_domains-1; d++) {
    ...
    if (d1 == pp->pp_data.N_domains) {
	 ...
	 wprintf_s("I can't find a parent domain for this domain\n");
	 print_domain(d);
	 exit(1); */
    }
  }
...
static int 
internal_process(Postprocessor *pp,Sublinkage *sublinkage,wchar_t **msg) 
{
  int i;
  ...
  if(!check_domain_nesting(pp, sublinkage->num_links)) 
      wprintf_s(L"WARNING: The domains are not nested.\n");
#endif
...
void prune_irrelevant_rules(Postprocessor *pp)
{
  ...
  for (coIDX=0;;coIDX++) 
   ...
   if (verbosity>1) {
     wprintf_s(L"Saw %i unique link names in all linkages.\n", 
	    pp_linkset_population(pp->set_of_links_of_sentence));
     wprintf_s(L"Using %i 'contains one' rules and %i 'contains none' rules\n", 
	    rcoIDX, rcnIDX);
   } 
}
...
/*
  ...
  matches anything, but "*" is just like an ordinary wchar_t for matching 
  purposes. For efficiency sake there are several different versions of these 
  functions 
  */

int post_process_match(wchar_t *s, wchar_t *t) 
{
	wchar_t c;
