...
int match(Connector *a, Connector *b, int aw, int bw) {
  /* 
	...
	assert(aw < bw, L"match() did not receive params in the natural order.");
	dist = effective_dist[aw][bw];
    }
    /*    wprintf_s("M: a=%4s b=%4s  ap=%d bp=%d  aw=%d  bw=%d  a->ll=%d b->ll=%d  dist=%d\n",
...
void table_update(int lw, int rw, Connector *le, Connector *re, int cost, int count) {
    ...
    assert(t != NULL, L"This entry is supposed to be in the table.");
    t->count = count;
}
...
int parse(Sentence sent, int cost, Parse_Options opts) {
    ...
    if (verbosity > 1) {
	wprintf_s(L"Total count with %d null links:   %d\n", cost, total);
    }
    if (total < 0) {
        wprintf_s(L"WARNING: Overflow in count!\n");
    }
