...
static wchar_t ** deletable;
static wchar_t ** effective_dist;
...
*/
	...
	assert(aw < bw, L"prune_match() did not receive params in the natural order.");
	dist = effective_dist[aw][bw];
    }
    /*    wprintf_s("PM: a=%4s b=%4s  ap=%d bp=%d  aw=%d  bw=%d  a->ll=%d b->ll=%d  dist=%d\n", 
...
*/
	if ((*(a->string)==L'S') && ((*s==L's') || (*s==L'p')) &&
	    ((*t==L'p') || (*t==L's')) &&
	    ((s-1 == a->string) || ((s-2 == a->string) && (*(s-1) == L'I')))){
	    return TRUE;
	}
/*
...
*/
	...
	if ((d->next->left == NULL) && (d->next->right == NULL)) {
	    d1 = d->next;
	    d->next = d1->next;
	    xfree((wchar_t *)d1, sizeof(Disjunct));
	} else {
	    d = d->next;
	}
    }
...
/* too big.  There are many fewer connectors than disjuncts.              */
	...
	if (verbosity > 2) {
	    wprintf_s(L"l->r pass removed %d\n",N_deleted);
	...
	if (verbosity > 2) {
	    wprintf_s(L"r->l pass removed %d\n",N_deleted);
	    print_disjunct_counts(sent);
	}
	free_S();
	if (N_deleted == 0) break;
    ...
    xfree((wchar_t *)table, s_table_size * sizeof (Connector *));
}
...
int string_hash(wchar_t * s, int i) {
/* hash function that takes a string and a seed value i */
    for(;*s != L'\0';s++) i = i + (i<<1) + randtable[(*s + i) & (RTSIZE-1)];
...
/* no use for d2. */    
    ...
    if ((e1!=NULL) || (e2!=NULL)) return FALSE;
    return (wcscmp(d1->string, d2->string) == 0);
}
...
*/
    ...
    xfree((wchar_t *) dup_table, dup_table_size * sizeof(Disjunct *));
    if ((verbosity > 2)&&(count != 0)) wprintf_s(L"killed %d duplicates\n",count);
    return d;
}
...
/*  
	...
	ne = e->u.l->e;
	xfree((wchar_t *) e->u.l, sizeof(E_list));
	xfree((wchar_t *) e, sizeof(Exp));
	return ne;
    }
*/    
...
*/
	...
	if (d->next->exp == NULL) {
	    d1 = d->next;
	    d->next = d1->next;
	    xfree((wchar_t *)d1, sizeof(X_node));
	} else {
	    d = d->next;
	}
    }
...
void expression_prune(Sentence sent){
	...
	for (w = 0; w < sent->length; w++) {
	    for (x = sent->word[w].x; x != NULL; x = x->next) {
/*     wprintf_s("before marking: "); print_expression(x->exp); wprintf_s("\n"); */
		N_deleted += mark_dead_connectors(x->exp, L'-');
/*     wprintf_s("after marking marking: "); print_expression(x->exp); wprintf_s("\n"); */
	    }
	    for (x = sent->word[w].x; x != NULL; x = x->next) {
/*     wprintf_s("before purging: "); print_expression(x->exp); wprintf_s("\n"); */
		x->exp = purge_Exp(x->exp);
/*     wprintf_s("after purging: "); print_expression(x->exp); wprintf_s("\n"); */
	    }
	    clean_up_expressions(sent, w);  /* gets rid of X_nodes with NULL exp */
	    for (x = sent->word[w].x; x != NULL; x = x->next) {
		insert_connectors(x->exp,L'+');
	    }
	}
...
/*     wprintf_s("before marking: "); print_expression(x->exp); wprintf_s("\n"); */
		N_deleted += mark_dead_connectors(x->exp, L'+');
/*     wprintf_s("after marking: "); print_expression(x->exp); wprintf_s("\n"); */
	    }
	    for (x = sent->word[w].x; x != NULL; x = x->next) {
/*     wprintf_s("before perging: "); print_expression(x->exp); wprintf_s("\n"); */
		x->exp = purge_Exp(x->exp);
/*     wprintf_s("after perging: "); print_expression(x->exp); wprintf_s("\n"); */
	    }
	    clean_up_expressions(sent, w);  /* gets rid of X_nodes with NULL exp */
	    for (x = sent->word[w].x; x != NULL; x = x->next) {
		insert_connectors(x->exp, L'-');
...
/* free all of the hash tables and C_lists */
	...
	xfree((wchar_t *)l_table[w], l_table_size[w] * sizeof (C_list *));
	for (i=0; i<r_table_size[w]; i++) {
	    free_C_list(r_table[w][i]);
	}
	xfree((wchar_t *)r_table[w], r_table_size[w] * sizeof (C_list *));
    }
}
...
/* The return value is the number of disjuncts deleted */
	...
	if (verbosity > 2) {
	   wprintf_s(L"l->r pass changed %d and deleted %d\n",N_changed,N_deleted);
	}
	...
	if (verbosity > 2) {
	   wprintf_s(L"r->l pass changed %d and deleted %d\n",N_changed,N_deleted);
	}
    ...
    if (verbosity > 2) wprintf_s(L"%d power prune cost:\n", power_cost);

    if (mode == RUTHLESS) {
	print_time(opts, L"power pruned (ruthless)");
    } else {
	print_time(opts, L"power pruned (gentle)");
    }
...
int match_in_cms_table(wchar_t * pp_match_name) {
...
Cms * lookup_in_cms_table(wchar_t * str) {
    Cms * cms;
    for (cms = cms_table[cms_hash(str)]; cms!=NULL; cms=cms->next) {
	if(wcscmp(str, cms->name) == 0) return cms;
    }
    return NULL;
}
...
/*
int is_in_cms_table(wchar_t * str) {
    Cms * cms = lookup_in_cms_table(str);
    return (cms != NULL && cms->count > 0);
}
*/

void insert_in_cms_table(wchar_t * str) {
    Cms * cms;
    int h;
...
int delete_from_cms_table(wchar_t * str) {
...
int pp_prune(Sentence sent, Parse_Options opts) {
	...
	for (w = 0; w < sent->length; w++) {
		...
		for (dir=0; dir < 2; dir++) {
			...
			for (i=0; i<knowledge->n_contains_one_rules; i++) {
			    ...
			    if (wcschr(selector, L'*') != NULL) continue;  /* If it has a * forget it */

			    if (!post_process_match(selector, c->string)) continue;

			    /*
			    wprintf_s("pp_prune: trigger ok.  selector = %s  c->string = %s\n", selector, c->string);
			    */
	...
	if (verbosity > 2) {
	   wprintf_s(L"pp_prune pass deleted %d\n", N_deleted);
	}
