...
void warning(Dictionary dict, wchar_t * s) {
    wprintf_s(L"\nWarning: %s\n",s);
    wprintf_s(L"line %d, current token = \"%s\"\n", dict->line_number, dict->token);
}
...
/* returns TRUE if this token is a special token and it is equal to c */
    return (dict->is_special && c==dict->token[0] && dict->token[1] == L'\0');
}
...
/* verbose version */
/*
int dict_compare(wchar_t *s, wchar_t *t) {
    int ss, tt;
    while (*s != '\0' && *s == *t) {
	s++;
	t++;
...
/* and right fields of it are NULL                                       */
    ...
    if (comp < 0) {
        ...
        swprintf_s(t, 128, L"The word \"%s\" has been multiply defined\n", new->string);
	dict_error(dict, t);
	return NULL;
    }
    return n;
...
/* It does the middle one first, then the left half, then the right.   */
    ...
    if (contains_underbar(dn->string)) {
		...
		wprintf_s(L"*** Word \"%s\" found near line %d.\n", dn->string, dict->line_number);
		wprintf_s(L"    Words ending \".Ix\" (x a number) are reserved for idioms.\n");
		wprintf_s(L"    This word will be ignored.\n");
		xfree((wchar_t *)dn, sizeof(Dict_node));	    
    } else if ((dnx = abridged_lookup(dict, dn->string))!= NULL) {
		wprintf_s(L"*** The word \"%s\"", dn->string);
		wprintf_s(L" found near line %d matches the following words:\n", dict->line_number);
		for (;dnx != NULL; dnx = dnx->right) {
			wprintf_s(L" %s", dnx->string);
		}
...
/* Add these words to the dictionary                                   */
    ...
    for (; !is_equal(dict, L':') ; advance(dict)) {
	  if (dict->is_special) {
	      dict_error(dict, L"I expected a word but didn\'t get it.");
	      return 0;
	  }
	  if (dict->token[0] == L'/') {
    ...
    if (!is_equal(dict, L';')) {
	dict_error(dict, L"Expecting \";\" at the end of an entry.");
	return 0;
    }
    if (!advance(dict)) {  /* pass the ; */
	return 0;
...
void print_expression(Exp * n) {
    E_list * el; int i;
    if (n == NULL) {
	wprintf_s(L"NULL expression");
	return;
    }
    if (n->type == CONNECTOR_type) {
	for (i=0; i<n->cost; i++) wprintf_s(L"[");
	wprintf_s(L"%s%c",n->u.string, n->dir);
	for (i=0; i<n->cost; i++) wprintf_s(L"] ");
    } else {
	for (i=0; i<n->cost; i++) wprintf_s(L"[");
	if (n->cost == 0) wprintf_s(L"(");
	if (n->type == AND_type) wprintf_s(L"& ");
	if (n->type == OR_type) wprintf_s(L"or ");
...
int dict_match(wchar_t * s, wchar_t * t) {
...
int true_dict_match(wchar_t * s, wchar_t * t) {
    wchar_t *ds, *dt;
    ds = wcsrchr(s, L'.');
    dt = wcsrchr(t, L'.');

    /* a dot at the end or a dot followed by a number is NOT considered a subscript */
    if ((dt != NULL) && ((*(dt+1) == L'\0') || (iswdigit((wint_t)*(dt+1))))) dt = NULL;
    if ((ds != NULL) && ((*(ds+1) == L'\0') || (iswdigit((wint_t)*(ds+1))))) ds = NULL;

    if (dt == NULL && ds != NULL) {
	if (((int)wcslen(t)) > (ds-s)) return FALSE;   /* we need to do this to ensure that */
	return (wcsncmp(s, t, ds-s) == 0);      /*"i.e." does not match "i.e" */
    } else if (dt != NULL && ds == NULL) {
	if (((int)wcslen(s)) > (dt-t)) return FALSE;   
	return (wcsncmp(s, t, dt-t) == 0);
    } else {
	return (wcscmp(s, t) == 0);	
    }
}
...
void prune_lookup_list(wchar_t * s) {
...
void free_lookup_list(void) {
	...
	n = lookup_list->right;
	xfree((wchar_t *)lookup_list, sizeof(Dict_node));
	lookup_list = n;
    }
}

void rdictionary_lookup(Dict_node * dn, wchar_t * s) {
...
Dict_node * dictionary_lookup(Dictionary dict, wchar_t *s) {
...
int boolean_dictionary_lookup(Dictionary dict, wchar_t *s) {
...
void rabridged_lookup(Dict_node * dn, wchar_t * s) {
    int m;
    Dict_node * dn_new;
    if (dn == NULL) return;
    m = dict_match(s, dn->string);
...
Dict_node * abridged_lookup(Dictionary dict, wchar_t *s) {
   free_lookup_list();
...
int boolean_abridged_lookup(Dictionary dict, wchar_t *s) {
...
int find_one_non_idiom_node(Dict_node * p, Dict_node * dn, wchar_t * s) {
...
void set_parent_of_node(Dictionary dict, 
	...
	if (p->left == del) {
	    ...
	    assert(FALSE, L"Dictionary broken?");
	}
    }
}

int delete_dictionary_words(Dictionary dict, wchar_t * s) {
...
*/
	...
	if (to_be_deleted->left == NULL) {
	    ...
	    xfree((wchar_t *)to_be_deleted, sizeof(Dict_node));
	} else {
	    ...
	    xfree((wchar_t *)pred, sizeof(Dict_node));
	}
...
int open_dictionary(wchar_t * dict_path_name, Dictionary dict) {
    if ((dict->fp = dictopen(dict_path_name, dict->name, L"r")) == NULL) {
	return 0;
    }
    return 1;
}
...
void free_Word_file(Word_file * wf) {
    ...
    for (;wf != NULL; wf = wf1) {
	wf1 = wf->next;
	xfree((wchar_t *) wf, sizeof(Word_file));
    }
}
...
void dict_display_word_info(Dictionary dict, wchar_t * s) {
	...
	wprintf_s(L"    \"%s\" matches nothing in the dictionary.\n", s);
	return;
    }
    wprintf_s(L"Matches:\n");
	...
	free_disjuncts(d1);
	wprintf_s(L"          ");
	left_print_string(stdout, dn->string, L"                  ");
	wprintf_s(L" %5d  ", len);
	if (dn->file != NULL) {
	    wprintf_s(L"<%s>", dn->file->file);
	}
	wprintf_s(L"\n");
    }
    free_lookup_list();
