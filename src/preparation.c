...
void free_deletable(Sentence sent) {
	...
	for (w = -1; w<sent->length; w++) {
	    xfree((wchar_t *)sent->deletable[w],sizeof(wchar_t)*(sent->length+1));
	}
	sent->deletable--;
	xfree((wchar_t *) sent->deletable, (sent->length+1)*sizeof(wchar_t *));
	sent->deletable = NULL;
    }
}
...
/* deletable.                                                              */
    ...
    free_deletable(sent);
    assert(sent->length < MAX_SENTENCE, L"sent->length too big");

    sent->deletable = (wchar_t **) xalloc((sent->length+1)*sizeof(wchar_t *));
    sent->deletable++;  /* we need to be able to access the [-1] position in this array */

    for (i = -1; i<sent->length; i++) {
	sent->deletable[i] = (wchar_t *) xalloc(sizeof(wchar_t)*(sent->length+1));  
	...
	for (j=0; j<= sent->length; j++) {
		...
		sent->deletable[i][j] = FALSE;
				 ...
				 sent->is_conjunction[j-1] ||
                                (wcscmp(L",",sent->word[i+1].string)==0 && 
				 conj_in_range(sent, i+2,j-1)) ||
 	                        (wcscmp(L",",sent->word[j-1].string)==0 && 
				 conj_in_range(sent, j,sent->length-1)))){
		sent->deletable[i][j] = TRUE;
	    } else if (j > i) {
		for (k=i+1; k<j; k++) {
		    if ((wcscmp(L"either", sent->word[k].string) == 0) ||
			(wcscmp(L"neither", sent->word[k].string) == 0) ||
			(wcscmp(L"both", sent->word[k].string) == 0) ||
			(wcscmp(L"not", sent->word[k].string) == 0)) continue;
		    if ((wcscmp(L"only", sent->word[k].string)==0) && (k > i+1) &&
                                   (wcscmp(L"not", sent->word[k-1].string)==0)) continue;
		    break;
		}
...
void free_effective_dist(Sentence sent) {
	...
	for (w=0; w<sent->length; w++) {
	    xfree((wchar_t *)sent->effective_dist[w],sizeof(wchar_t)*(sent->length+1));
	}
	xfree((wchar_t *) sent->effective_dist, sizeof(wchar_t *)*(sent->length));
...
void build_effective_dist(Sentence sent, int has_conjunction) {
  ...
  */
    ...
    sent->effective_dist = (wchar_t **) xalloc(sent->length*sizeof(wchar_t *));

    for (i=0; i<sent->length; i++) {
	sent->effective_dist[i] = (wchar_t *) xalloc(sizeof(wchar_t)*(sent->length+1));
    }
    for (i=0; i<sent->length; i++) {  
    ...
    for (i=0; i<sent->length; i++) {  
      for (j=0; j<=sent->length; j++) {
	wprintf_s("%4d", sent->effective_dist[i][j]);
      }
      wprintf_s("\n");
    }
    */
...
/* sentence */  
	...
	if (sent->is_conjunction[i]) {
	    ...
	    if (wcscmp(sent->word[i].string, L",") == 0) {
		sent->word[i].d = catenate_disjuncts(sent->word[i].d,
...
static void set_connector_list_length_limit(Connector *c, Connector_set *conset, int short_len, Parse_Options opts) {
	...
	else if (conset == NULL || match_in_connector_set(conset, c, L'+')) {
...
/* structures.                                                        */
    ...
    if (verbosity > 2) {
	wprintf_s(L"After expanding expressions into disjuncts:") ;
	print_disjunct_counts(sent);
    }
    print_time(opts, L"Built disjuncts");
    
    for (i=0; i<sent->length; i++) {
	sent->word[i].d = eliminate_duplicate_disjuncts(sent->word[i].d);
    }
    print_time(opts, L"Eliminated duplicate disjuncts");
	...
	*/
	...
	print_time(opts, L"Constructed fat disjuncts");
	
	prune(sent);
	print_time(opts, L"Pruned fat disjuncts");
	
	for (i=0; i<sent->length; i++) {
	    sent->word[i].d = eliminate_duplicate_disjuncts(sent->word[i].d);
	}
	if (verbosity > 2) {
	    wprintf_s(L"After pruning and duplicate elimination:\n");
	    print_disjunct_counts(sent);
	}
	print_time(opts, L"Eliminated duplicate disjuncts (again)");
    ...
    */
    ...
    print_time(opts, L"Initialized fast matcher and hash table");
}
