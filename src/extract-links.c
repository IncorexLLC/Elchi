...
void init_x_table(Sentence sent) {
    ...
    assert(sent->parse_info == NULL, L"Parse_info is not NULL");
    ...
    /*wprintf_s("Allocating x_table of size %d\n", x_table_size);*/
...
/* This is the function that should be used to free tha set structure. Since
   ...
   a set element, we put it in the hash table, so this is OK.*/
    ...
    if (pi->x_table == NULL) {
	/*fwprintf_s(stderr, "Warning: Tried to free a NULL x_table\n");*/
	return;
    }
    ...
    /*wprintf_s("Freeing x_table of size %d\n", x_table_size);*/
...
void x_table_update(int lw, int rw, Connector *le, Connector *re, 
    ...
    assert(t != NULL, L"This entry is supposed to be in the x_table.");
    t->set = set;
}
...
Parse_set * parse_set(Disjunct *ld, Disjunct *rd, int lw, int rw, 
    ...
    assert(cost >= 0, L"parse_set() called with cost < 0.");
...
int verify_set_node(Parse_set *set) {
    ...
    assert(n == set->count, L"verify_set failed");
    return (n < 0) || (n != (int) dn);
}
...
int verify_set(Parse_info * pi) {
    ...
    overflowed=FALSE;
    assert(pi->x_table != NULL, L"called verify_set when x_table==NULL");
    for (i=0; i<pi->x_table_size; i++) {
...
void issue_link(Parse_info * pi, Disjunct * ld, Disjunct * rd, struct Link_s link) {
    assert(pi->N_links <= MAX_LINKS-1, L"Too many links"); 
...
void list_links(Parse_info * pi, Parse_set * set, int index) {
     ...
     assert(pc != NULL, L"walked off the end in list_links");
...
void list_random_links(Parse_info * pi, Parse_set * set) {
     ...
     assert(pc != NULL, L"Couldn't get a random parse choice");
     issue_links_for_choice(pi, pc);
