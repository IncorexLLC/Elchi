...
struct Tconnector_struct{
    ...
    wchar_t dir;     /* '-' for left and '+' for right */
    Tconnector * next;
    wchar_t * string;
};
...
void free_Tconnectors(Tconnector *e) {
    ...
    for(;e != NULL; e=n) {
	n = e->next;
	xfree((wchar_t *)e, sizeof(Tconnector));
    }
}
...
/* Build the clause for the expression e.  Does not change e */
    ...
    assert(e != NULL, L"build_clause called with null parameter");
    if (e->type == AND_type) {
	...
	assert(FALSE, L"an expression node with no type");
    }
...
void print_connector_list(Connector * e) {
    for (;e != NULL; e=e->next) {
	wprintf_s(L"%s",e->string);
	if (e->label != NORMAL_LABEL) {
	    wprintf_s(L"%3d", e->label);
	} else {
	    wprintf_s(L"   ");
	}
	if (e->next != NULL) wprintf_s(L" ");
    }
}

void print_Tconnector_list(Tconnector * e) {
    for (;e != NULL; e=e->next) {
	if (e->multi) wprintf_s(L"@");
	wprintf_s(L"%s",e->string);
	wprintf_s(L"%c", e->dir);
	if (e->next != NULL) wprintf_s(L" ");
    }
...
void print_clause_list(Clause * c) {
    for (;c != NULL; c=c->next) {
	wprintf_s(L"  Clause: ");
	wprintf_s(L"(%2d, %2d)", c->cost, c->maxcost);
...
void print_disjunct_list(Disjunct * c) {
    for (;c != NULL; c=c->next) {
	wprintf_s(L"%10s: ", c->string);
	wprintf_s(L"(%2d)", c->cost);
...
Connector * extract_connectors(Tconnector *e, wint_t c) {
...
Disjunct * build_disjunct(Clause * cl, wchar_t * string, int cost_cutoff) {
...
/* string is the print name of word that generated this disjunct */
	...
	if (cl->maxcost <= cost_cutoff) {
	    ...
	    ndis->left = reverse(extract_connectors(cl->c, L'-'));
	    ndis->right = reverse(extract_connectors(cl->c, L'+'));
	    ndis->string = string;
	    ndis->cost = cl->cost;
	    ndis->next = dis;
	    dis = ndis;
...
#if 0
OBS 
OBS Disjunct * build_word_disjuncts(wchar_t * s) {
...
X_node * build_word_expressions(Sentence sent, wchar_t * s) {
