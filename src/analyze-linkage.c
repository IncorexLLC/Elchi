...
{
    wchar_t used;   /* TRUE if this link is used, else FALSE  */
    wchar_t changed;/* TRUE if this link changed, else FALSE  */
...
struct Links_to_patch_struct {
    Links_to_patch * next;
    int link;
    wchar_t dir;   /* this is 'r' or 'l' depending on which end of the link
		   is to be patched. */
...
static Sublinkage * x_create_sublinkage(Parse_info * pi) {
    ...
    assert(pi->N_links < MAX_LINKS, L"Too many links");
    return s;
}
...
static void replace_link_name(Link l, wchar_t *s) {
    exfree(l->name, sizeof(wchar_t)*(wcslen(l->name)+1));
    l->name = (wchar_t *) exalloc(sizeof(wchar_t)*(wcslen(s)+1));
    wcscpy(l->name, s);
}
...
/* get down the tree past all the commas */
	...
	if (lol->dir == 1) {
	    ...
	    if (wcscmp(sent->word[lol->word].string, L",")==0) {
		    /* pointing to a comma */
...
static int strictly_smaller_name(wchar_t * s, wchar_t * t) {
/*
...
Linkage_info analyze_fat_linkage(Sentence sent, Parse_Options opts, int analyze_pass) {
  /* This uses link_array.  It enumerates and post-processes
    ...
    /* if (display_on && (li.N_violations != 0) && 
       ...
       wprintf_s("P.P. violation in one part of conjunction.\n"); */
    free_sublinkage(sublinkage);
    free_digraph(pi);
    free_DIS_tree(d_root);
    return li;
