...
int main(){
    ...
    while (my_fget_input_string(input_string, stdin, stdout, opts)) {
		...
		if (num_linkages == 0) {
			wprintf_s(L"!top ERROR: No linkage found for input sentence\n");
			if (localv == 3)
		...
		/* wprintf_s("Number of words = %d\n", numwords);  */

		in_alinkset = build_input_linkset(linkage);
		if(localv == 3)
			print_input_blurb(linkage, in_alinkset);
		...
		if(localv == 2)
			wprintf_s(L"After choosing correct wordforms:\n");
		if(localv == 2)
			print_links(out_alink);
		...
		if(linkage->info.fat==TRUE) {
			...
			if(ok == 0)
				continue;
			if(localv == 3)
				print_conjunction_blurb(out_alinkset);
		}

		if(localv == 2)
			wprintf_s(L"Transformations completed\n\nGenerating new sentence...\n"); 

		for (al=out_alink; al!=NULL; al=al->next) {
			if(wcscmp(al->left, L"LEFT-WALL")==0) {
		...
		if(linkage_found == 0) {
			wprintf_s(L"!top ERROR: No translation found. (The translator cannot handle some of the word usages \n!top in your sentence.)\n");
			if(localv == 3)
				print_nooutlinkage_error();
			free_alinkset(in_alinkset);
		...
		if(linkage->info.fat==TRUE)
			insert_conjunction_commas(out_alinkset);
      
		linkage_found = print_words_in_order(out_alink);

		if(linkage_found == 0)
			wprintf_s(L"!top ERROR: No linkage found for output linkset. (Words in\n output linkset are not fully connected.)\n");      
    ...
    if (space_in_use != 0) {
        fwprintf(stderr, L"Warning: %d bytes of space leaked.\n", space_in_use);
    }
    
	if (external_space_in_use != 0) {
        fwprintf(stderr, L"Warning: %d bytes of external space leaked.\n", external_space_in_use);
    }
    return 0;
}
...
Alinkset * build_input_linkset(Linkage linkage) {
    ...
    if(localv == 2 && linkage->info.fat == TRUE) {
		for(andlist = linkage->info.andlist; andlist!=NULL; andlist=andlist->next) {
			wprintf_s(L"And-list, c=%d: ", andlist->conjunction);
			for(i = 0; i < andlist->num_elements; i++) {
				wprintf_s(L"%d ", andlist->element[i]);
			}
			wprintf_s(L"\n");
		}
    }
    ...
    for(i=0; i<linkage->num_words; i++) {
		...
		if(tword[i].conj == -1)
			tword[i].conj = i;
		/* wprintf_s("word %d = %s, conj = %d\n", i, tword[i].estring, tword[i].conj);  */
    }

    if(localv == 2)
		wprintf_s(L"INPUT LINKS:\n");
    ...
    for(i=0; i<linkage_get_num_links(linkage); i++) {
		...
		front_alink = nalink;
		if(localv == 2)
			wprintf_s(L"  Link %d: %s (%d) %s %s (%d)\n", i, nalink->left, 
			nalink->leftsub, nalink->connector, nalink->right, nalink->rightsub);   		
    }
...
void modify_input_linkset(Alinkset * alinkset) {
    ...
    if(trans1 == 1) {
		if(localv == 2) wprintf_s(L"After modifying relative clause:\n");
    ...
    if(trans2 == 1 || trans3 == 1) {
		if(localv == 2) wprintf_s(L"After removing auxiliary:\n");
		if(localv == 2) print_links(alink);
    }
    ...
    if(trans4 == 1) {
		if(localv == 2) wprintf_s(L"After genitive transform:\n");
		if(localv == 2) print_links(alink);
    }

    trans5 = add_that(alink);
    if(trans5 == 1) {
		if(localv == 2) wprintf_s(L"After adding 'that' before embedded clause:\n");
		if(localv == 2) print_links(alink);
    }
...
int remove_do_transform(Alink * alink) {
  ...
  /* This function takes the input (English) linkset as input. It looks to see if there's
    ...
    ok=0;
    for(al = alink; al!=NULL; al=al->next) {
		if((uppercompare(al->connector, L"SI") == 0 && (wcscmp(al->left, L"do.v")==0 || wcscmp(al->left, L"does.v")==0 || wcscmp(al->left, L"did.v")==0)) ||
		(uppercompare(al->connector, L"S") == 0 && (wcscmp(al->right, L"do.v")==0 || wcscmp(al->right, L"does.v")==0 || wcscmp(al->right, L"did.v")==0))) {
			ok=1;
...
int genitive_transform(Alink * alink) {
    ...
    for(al = alink; al!=NULL; al=al->next) {
		...
		if(wcscmp(al->connector, L"YS")==0 || wcscmp(al->connector, L"YP")==0) {  
			wcscpy(ds1, al->left);
			if(iswupper(ds1[0])) {
				wprintf_s(L"%s has cap first letter\n", al->left);
				continue;
			}
			for(al2 = alink; al2!=NULL; al2=al2->next) {
...
int modify_relative_clause(Alink * alink) {
    ...
    for(al = alink; al!=NULL; al=al->next) {
      if(wcscmp(al->connector, L"Bs")==0 || wcscmp(al->connector, L"Bp")==0) {
	al->ignore=1;
	trans_performed=1;
      }
    }
	...
	if(localv == 2) if(ok==0) wprintf_s(L"Warning: no S link found in object-type relative clause");
      }
    }
...
Alink * replace_words_dumbly(Dictionary e_table, Dictionary g_table, Alink * alink) {
	...
	if(localv == 2) wprintf_s(L"Error: an estring that is supposed to be filled is null\n");
	return NULL;
      }
      wcscpy(ds1, tword[i].estring);

      /* wprintf_s("Looking up %s in e_table\n", ds1); */
	...
	/* If there's no matching word in the English table, just go on to the next word */
      ...
      cs1 = d1->right->string;                                   /* We've looked up a word and found the table entry and the
							   connector */
      /* wprintf_s("Found category connector for %s: %s\n", ds1, d1->right->string); */
	...
	if(x==1) {
      ...
      if(localv == 2) if (x==1) wprintf_s(L"  Replaced '%s' with '%s'\n", ds1, ws); 
      if(localv == 2) if (x==0) wprintf_s(L"  Not replacing '%s'\n", ds1);
	...
	if(side==0) {
	  if(tword[al->leftsub].gstring==NULL) {
	    wprintf_s(L"!top ERROR: No match found for word '%s'; sentence cannot be translated\n", tword[al->leftsub].estring);
    ...
    if(localv==2) wprintf_s(L"After dumb word replacement:\n");
...
Alinkset * replace_words(Dictionary e_table, Dictionary g_dict, Dictionary g_table, Alinkset * in_alinkset) {
    ...
    if(sentence_ok == 0) {
      if(localv==2) wprintf_s(L"Error: problem with setting case values\n");
      return NULL;
    }
    ...
    if(localv == 2) wprintf_s(L"After choosing participles:\n");
    ...
    for(al=out_alink; al!=NULL; al=al->next) {
      ...
      if(wcscmp(tword[al->leftsub].estring, L"when")==0) {
    ...
    if(sentence_ok = 0) {
      wprintf_s(L"!top ERROR: Sentence cannot be translated; valid wordforms could not be found\n");
...
void split_separable_words(Dictionary g_dict, Alink * alink, int w) {
    ...
    for(c = bigword; *c!=L'\0'; c++) {
      if(*c==L'+') {
	if(localv == 2) wprintf_s(L"separable word found: '%s'\n", tword[w].gstring);
	wcsncpy(firstword, bigword, c-bigword);
	firstword[c-bigword]=L'\0';
	for(c2 = c; *c2!=L'\0'; c2++);
	wcsncpy(secondword, c+1, c2-(c+1));

	if(localv == 2) wprintf_s(L"split it into two words: '%s' and '%s'\n", firstword, secondword);
      }
    }
    ...
    if(localv == 2) wprintf_s(L"After separable-word transformation:\n");
    if(localv == 2) print_links(alink);
}
...
int set_case_values(Alink * out_alink, Dictionary g_table, Dictionary g_dict) {
    ...
    for(al=out_alink; al!=NULL; al=al->next) {
		if(uppercompare(al->connector, L"S")==0)
			tword[al->leftsub].xcase=1;
		if(uppercompare(al->connector, L"SI")==0)
			tword[al->rightsub].xcase=1;
		if(uppercompare(al->connector, L"MX")==0)
			tword[al->rightsub].xcase=1;
		if(uppercompare(al->connector, L"O")==0)
			tword[al->rightsub].xcase=2;
		if(uppercompare(al->connector, L"J")==0) {
			cs = find_connector_on_word(g_dict, al->left, L"J", 1, al->leftsub);
			if(cs == NULL)
				return 0;
			if(post_process_match(L"J#d", cs)==1)
				tword[al->rightsub].xcase = 3;
			if(post_process_match(L"J#a", cs)==1)
				tword[al->rightsub].xcase = 2;
		}
		if(uppercompare(al->connector, L"GE")==0)
			tword[al->rightsub].xcase=4;
		if(uppercompare(al->connector, L"D")==0) {
			dn = dictionary_lookup(g_table, al->left);
			if(word_has_connector(dn, L"DEF", 0)==1)
				tword[al->rightsub].def=1;
			else if(word_has_connector(dn, L"INDEF", 0)==1)
				tword[al->rightsub].def=2;
			else{
				/*Here the below line Esen added*/
				if (cs != NULL)
					xfree(cs, wcslen(cs)*sizeof(wchar_t));
				return 0;
			}
		}
    }
...
int choose_participles(Dictionary g_dict, Dictionary g_table, Alink * alink, Alink * al, int type) {
  ...
  /* This function takes a link as input; the link is either an I link (type = 1), a PP (type = 2), a
	...
	if(localv == 2) if(ok == -1) wprintf_s(L" Warning: The word '%s' is not in the dictionary\n", ws);
...
wchar_t * find_connector_on_word(Dictionary dict, wchar_t * w, wchar_t * cs, int direction, int wordsub) {  
    ...
    /* wprintf_s("Checking for %s\n", cs); */
    for (;dn != NULL; dn = dn->right) {
		/* wprintf_s("Trying node %s\n", dn->string); */
		...
		for(; d!=NULL; d=d->next) {
			...
			if(tword[wordsub].xcase != 0 && (uppercompare(cs, L"D")==0 || uppercompare(cs, L"A")==0)) {
				...
				for (c = d->right; c!=NULL; c=c->next) {
					if(tword[wordsub].xcase==1 && uppercompare(c->string, L"S")==0)
						ok = 1;
				}
				for (c = d->left; c!=NULL; c=c->next) {
					if(tword[wordsub].xcase==2 && (uppercompare(c->string, L"O")==0 || easy_match(c->string, L"J*a")==1))
						ok = 1;
				}
				for (c = d->left; c!=NULL; c=c->next) {
					if(tword[wordsub].xcase==3 && easy_match(c->string, L"J*d")==1)
						ok = 1;
				}
				for (c = d->left; c!=NULL; c=c->next) {
					if(tword[wordsub].xcase==4 && easy_match(c->string, L"GE")==1)
						ok = 1;
				}
				if(ok == 0)
					continue;
			...
			if(uppercompare(cs, L"D")==0 || uppercompare(cs, L"A")==0) {
				ok = 1;
				/* wprintf_s("for fixed word '%s', def = %d\n", w, tword[wordsub].def); */
				for (c = d->left; c!=NULL; c=c->next) {
					...
					if(tword[wordsub].def==1 && post_process_match(L"D##i", c->string)==1)
						ok = 0;
					if(tword[wordsub].def==2 && post_process_match(L"D##d", c->string)==1)
						ok = 0;
				}
			...
			for(; c != NULL; c = c->next) {
				if(easy_match(c->string, cs) == 1) {
					wchar_t *string_p = (wchar_t*)xalloc(wcslen(c->string)*sizeof(wchar_t));
					wcscpy(string_p, c->string);
					free_disjuncts(d0);
					return string_p;         /* This does a smart match */
				}
			}
		}
    }
...
int choose_correct_wordforms(Dictionary g_dict, Dictionary g_table, Alinkset * alinkset, Alink * al, int direction) {
    ...
    safe_strcpy(cs2, al->connector, 100);
    for(i=0; cs2[i] != L'\0'; i++) {
		if (!iswupper(cs2[i]))
			cs2[i] = L'\0'; /* terminate at first non capital */
    }
    ...
    if((direction == 1 && (uppercompare(cs2, L"SI")==0 || uppercompare(cs2, L"O")==0)) ||
       (direction == 0 && uppercompare(cs2, L"S")==0)) {   /* why doesn't wcscmp work here? */
	 ...
	 subscript to determine case.) */
      if(localv == 2) wprintf_s(L"Replacing '%s': Find a noun of the right case\n", tword[tw].gstring);
      cs1 = cs2;
    }
      ...
      cs1 = string_set_add(L"Sp", translator_strings);
		...
		if(localv == 2)
			wprintf_s(L"Replacing '%s': '%s' has a %s; find a form with a matching connector\n", tword[tw].gstring, tword[fw].gstring, cs1);
    }

    if (cs1 == NULL) {
		if(localv == 2)
			wprintf_s(L"No matching connector found\n");
		return 0;
    }
	...
	if(localv == 2) if(ok == -1) wprintf_s(L"  Warning: The word '%s' isn't in the dictionary\n", ws);

	if(ok==1 && uppercompare(cs1, L"R")==0) {
	  if(localv == 2) wprintf_s(L"The word '%s' matches the antecedent\n", ws);
	  ...
	  if(localv == 2) if(ok==0) wprintf_s(L" ...but it's the wrong case for the relative clause\n");
	  if(localv == 2) if(ok==1) wprintf_s(L" ...and it's the right case for the relative clause\n");
	}

	if(ok==1) {
	  if(localv == 2) wprintf_s(L"  The word '%s' has a match\n", ws);

	  if(tword[tw].tense==1) cs3 = present;
	  if(tword[tw].tense==2) cs3 = past;
	  if(tword[tw].tense!=0) {
	    ok = word_has_connector(dn2, cs3, 1);
	    if(localv == 2) if(ok == 0) wprintf_s(L"  ...but it doesn't have the right tense\n");
	    if(localv == 2) if(ok == 1) wprintf_s(L"  ...and it has the right tense\n", ws);
	  }
	} 
	...
	if(localv == 2) if(ok==0 && tword[tw].tense==0) wprintf_s(L"  Word '%s' doesn't have match\n", ws);
      }
    }
    free_listed_dictionary(dn4);
    return 0;
...
int choose_correct_linktype(Dictionary g_dict, Alinkset * alinkset, Alink * al, int direction) {
    ...
    /* Replace the connector in the link with unsubscripted form */
    safe_strcpy(cs2, al->connector, /*sizeof(cs2)*/100);
    for(i=0; cs2[i] != L'\0'; i++) {
	if (!iswupper(cs2[i])) cs2[i] = L'\0'; /* terminate at first non capital */
    }
    al->connector = string_set_add(cs2, translator_strings);    
    ...
    cs1 = find_connector_on_word(g_dict, tword[fw].gstring, cs2, direction, fw);
    if(localv == 2) wprintf_s(L"Replacing '%s' link: using %s\n", tword[tw].gstring, cs1);
    ...
    if (cs1==NULL) {
      if(localv == 2) wprintf_s(L"No matching connector found\n");
      return 0;
    }
...
void apply_transformations(Alinkset * out_alinkset) {
    ...
    if(transform_performed == 1) {
      if(localv == 2) wprintf_s(L"After switching head:\n");
    ...
    for(al=out_alinkset->link; al!=NULL; al=al->next) {
      if(uppercompare(al->connector, L"TO")==0 || wcscmp(al->connector, L"MVi")==0) {
    ...
    for(al=out_alinkset->link; al!=NULL; al=al->next) {
      if(wcscmp(al->connector, L"MVs")==0 || wcscmp(al->connector, L"TH")==0) {
    ...
    for(al=out_alinkset->link; al!=NULL; al=al->next) {
	 ...
	 uppercompare(al->connector, L"P")==0 || uppercompare(al->connector, L"PV")==0 || wcscmp(al->connector, L"SCz")==0) {
...
void adjust_infinitive_clause(Alink * alink, Alink * al) {
    ...
    if(localv == 2) wprintf_s(L"After modifying infinitive:\n");
    if(localv == 2) print_links(alink);
}
...
void reattach_subordinate_clause(Alink * alink, Alink * al) {
  ...
  /* This function reattaches a subordinate clause from a participle to the finite verb of the main clause. 
    ...
    if(wcscmp(al->connector, L"MVs")==0) al->connector = string_set_add(L"SC", translator_strings);  
    if(wcscmp(al->connector, L"TH")==0) {
      add_commas(alink, al, 1);
    ...
    if(localv == 2) wprintf_s(L"After reattaching subordinate clause:\n");
    if(localv == 2) print_links(alink);
}
...
void put_participle_at_end(Alink * alink, Alink * al) {
    ...
    for(al2=alink; al2!=NULL; al2=al2->next) {
      if(al2->leftsub == al->rightsub) {
	if(wcscmp(al2->connector, L"Xc")==0) continue;
    ...
    if(localv == 2) wprintf_s(L"After putting participle at end:\n");
    if(localv == 2) print_links(alink);
}
...
void s_v_inversion(Alink * alink, Alink * al) {
  ...
  /* This function is only called if there's a CO link in the input sentence. The S linking
    ...
    al->leftsub = tempnum;
    al->connector = string_set_add(L"SI", translator_strings);
    if(localv == 2) wprintf_s(L"After subject-verb inversion:\n");
    if(localv == 2) print_links(alink);
}
...
void put_finite_verb_at_end(Alink * alink, Alink * al) {
  ...
  /* This takes a C link as input (al). The right end of the C has already been switched to the finite verb. 
    ...
    if(al2==NULL) {
      wprintf_s(L"Error! No S was found at the right end of a C / RO connector\n");
      return;
    }
    ...
    if(localv == 2) wprintf_s(L"After putting the finite verb on the end of the subordinate clause:\n");
    if(localv == 2) print_links(alink);
}
...
int generate_fat_linkage(Linkage linkage, Alink * alink) {
    ...
    for(andlist = linkage->info.andlist; andlist!=NULL; andlist=andlist->next) {
		if(tword[andlist->conjunction].gstring == NULL) {
			wprintf_s(L"!top ERROR: The conjunction '%s' is not understood by the translator.\n", tword[andlist->conjunction].estring);
			return 0;
		}
		...
		if(localv==2)
			wprintf_s(L"Rebuilding outside-world list for andlist (conj=%d): ", andlist->conjunction);
		...
		for(al=alink; al!=NULL; al=al->next) {
			...
			/*	    for(i=0; i<andlist->num_outside_words; i++) if(al2->rightsub==andlist->outside_word[i]) continue; */
					...
					if(localv==2)
						wprintf_s(L"%d ", tword[al->rightsub].conj);
				...
				if(al2->leftsub == al->leftsub && tword[al2->rightsub].conj==andlist->conjunction &&
					...
					if(localv == 2)
						wprintf_s(L"%d ", tword[al->leftsub].conj);
					++andlist->num_outside_words;
				}
			}
		}
		...
		for(i = 0; i < andlist->num_elements; ++i) {
			...
			if(andlist->element[i]<andlist->conjunction) {
				...
				newlink = create_new_link(alink, tword[andlist->element[i]].gstring, andlist->element[i],
					tword[andlist->conjunction].gstring, andlist->conjunction, connector);
			} else {
		...
		if(localv == 2)
			wprintf_s(L"After adding fat links:\n");
		if(localv == 2)
			print_links(alink);
	...
	for(j=0; j<andlist->num_elements; j++) {
	  for(al=alink; al!=NULL; al=al->next) {
		...
		if(localv == 2) wprintf_s(L"Outside link generated to %d\n", andlist->outside_word[i]); 
	      }
	      else al->ignore = 1;
	    }
		...
		outside_link_generated = 1;
							if(localv == 2)
								wprintf_s(L"Outside link generated to %d\n", andlist->outside_word[i]);
						}
						else
							al->ignore = 1;
					}
		...
		if(localv == 2)
			wprintf_s(L"After adding links to outside world:\n");
		if(localv == 2)
			print_links(alink);
    }
    return 1;
...
void print_links(Alink * alink) {
    ...
    for(al = alink; al != NULL; al = al->next) {
		if(al->ignore == 1) continue;
		wprintf_s(L"  %12s (%2d) %4s  (%2d) %s \n", al->left, 
			al->leftsub, al->connector, al->rightsub, al->right);   		
    }
}

int uppercompare(wchar_t * s, wchar_t * t) {
	while(iswupper(*s) || iswupper(*t)) {
		if (*s != *t) return 1;
		s++;
		t++;
	}
...
void remove_duplicate_links(Alink * alink) {
    ...
    for(al = alink; al != NULL; al = al->next) {
		for(al2 = al; al2!=NULL; al2=al2->next) {
			if(al2->left == al->left && al2->right == al->right)
				al2->ignore == 1;
		}
    }
}
...
void free_alinkset(Alinkset * alinkset) {
	...
	for (al = alinkset->link; al != NULL; al = al2) {
		al2 = al->next;
