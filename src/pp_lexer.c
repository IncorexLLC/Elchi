 ...
 * we want to instead treat it as an 8-bit wchar_t, hence the
 * double cast.
 */
...
/* Action number for WEOF rule of a given start state. */
...
#define YY_BUFFER_NORMAL 1
	/* When an WEOF's been seen but there's still some text to process
	 * then we mark the buffer as YY_EOF_PENDING, to indicate that we
	 * shouldn't try reading from the input source any more.  We might
	 * still have a bunch of tokens to match, though, because of
	 * possible backing-up.
	 *
	 * When we actually see the WEOF, we change the status to "new"
...
static wchar_t *yy_c_buf_p = (wchar_t *) 0;
...
YY_BUFFER_STATE yy_scan_buffer YY_PROTO(( wchar_t *base, yy_size_t size ));
YY_BUFFER_STATE yy_scan_string YY_PROTO(( const wchar_t *str ));
YY_BUFFER_STATE yy_scan_bytes YY_PROTO(( const wchar_t *bytes, int len ));
...
#if 0  /* --DS */
#ifndef YY_NO_UNPUT
static void yyunput YY_PROTO(( int c, wchar_t *buf_ptr ));
#endif
#endif /* --DS */

#ifndef yytext_ptr
static void yy_flex_strncpy YY_PROTO(( wchar_t *, const wchar_t *, int ));
#endif
...
static void yy_fatal_error YY_PROTO(( const wchar_t msg[] ));
...
extern void error(wchar_t *fmt, ...);

/* forward references for non-exported functions (and static variable) */
static void check_string(const wchar_t *str);
static void setup(PPLexTable *lt);
static void set_label(PPLexTable *lt, const wchar_t *label);
static void add_string_to_label(PPLexTable *lt, wchar_t *str);
static void add_set_of_strings_to_label(PPLexTable *lt,const wchar_t *label_of_set);
/* static void show_bindings(PPLexTable *lt); --DS */
static int  get_index_of_label(PPLexTable *lt, const wchar_t *label);
...
#define YY_INPUT(buf, result, max_size) \
	...
	else if ( ((result = fread( buf, sizeof(wchar_t), max_size, yyin )) == 0) \
		  && ferror( yyin ) ) \
		YY_FATAL_ERROR( L"input in flex scanner failed" );
#endif
...
do_action:	/* This label is used only to access WEOF actions. */
...
{ error(L"pp_lexer: unable to parse knowledge file (line %i).\n",yylineno); }
	YY_BREAK
case 8:
YY_RULE_SETUP
...
static int yy_get_next_buffer()
	{
	register wchar_t *dest = yy_current_buffer->yy_ch_buf;
	register wchar_t *source = yytext_ptr;
	...
	if ( yy_current_buffer->yy_fill_buffer == 0 )
		{ /* Don't try to fill the buffer, so this is an WEOF. */
		if ( yy_c_buf_p - yytext_ptr - YY_MORE_ADJ == 1 )
			{
			/* We matched a singled characater, the EOB, so
			 * treat this as a final WEOF.
			 */
	...
	if ( yy_current_buffer->yy_buffer_status == YY_BUFFER_EOF_PENDING )
		/* don't do the read, it's not guaranteed to return an WEOF,
		 * just force an WEOF
		 */
		yy_n_chars = 0;
			...
			if ( b->yy_is_our_buffer )
				...
				b->yy_ch_buf = (wchar_t *)
					...
					yy_flex_realloc( (void *) b->yy_ch_buf,
							 (b->yy_buf_size + 2)*sizeof(wchar_t) ); // sizeof I added
				}
			else
...
#if 0  /* --DS */
#ifdef YY_USE_PROTOS
static void yyunput( int c, register wchar_t *yy_bp )
#else
static void yyunput( c, yy_bp )
int c;
register wchar_t *yy_bp;
#endif
	{
	register wchar_t *yy_cp = yy_c_buf_p;
...
#ifdef __cplusplus
					YY_FATAL_ERROR(
					L"unexpected last match in yyinput()" );
#else
					YY_FATAL_ERROR(
					L"unexpected last match in input()" );
#endif
				}
			}
		}

	c = *(wchar_t *) yy_c_buf_p;	/* cast for 8-bit wchar_t's */
	*yy_c_buf_p = L'\0';	/* preserve yytext */
	yy_hold_char = *++yy_c_buf_p;

	yy_current_buffer->yy_at_bol = (c == L'\n');
...
YY_BUFFER_STATE new_buffer;
	 ...
	 * WEOF (yywrap()) processing, but the only time this flag
...
int size;
	 ...
	 */
	b->yy_ch_buf = (wchar_t *) yy_flex_alloc(sizeof(wchar_t)*(b->yy_buf_size + 2));
	if ( ! b->yy_ch_buf )
		YY_FATAL_ERROR( L"out of dynamic memory in yy_create_buffer()" );
...
#ifdef YY_USE_PROTOS
YY_BUFFER_STATE yy_scan_buffer( wchar_t *base, yy_size_t size )
#else
...
#ifdef YY_USE_PROTOS
YY_BUFFER_STATE yy_scan_string( const wchar_t *str )
#else
...
#ifdef YY_USE_PROTOS
YY_BUFFER_STATE yy_scan_bytes( const wchar_t *bytes, int len )
#else
...
int len;
	...
	n = len + 2;
	buf = (wchar_t *) yy_flex_alloc( sizeof(wchar_t)*n );
	if ( ! buf )
...
int new_state;
	...
	if ( yy_start_stack_ptr >= yy_start_stack_depth )
		...
		if ( ! yy_start_stack )
			YY_FATAL_ERROR(
			L"out of memory expanding start-condition stack" );
		}
...
#ifdef YY_USE_PROTOS
static void yy_fatal_error( const wchar_t msg[] )
#else
static void yy_fatal_error( msg )
wchar_t msg[];
#endif
	{
	(void) fwprintf_s( stderr, L"%s\n", msg );
...
#ifndef yytext_ptr
#ifdef YY_USE_PROTOS
static void yy_flex_strncpy( wchar_t *s1, const wchar_t *s2, int n )
#else
...
{
  PPLexTable *lt;
  if (f==NULL) error(L"pp_lexer_open: passed a NULL file pointer");
...
int pp_lexer_count_tokens_of_label(PPLexTable *lt) 
{
  ...
  if (lt->idx_of_active_label==-1) 
	error(L"pp_lexer: current label is invalid");
...
int pp_lexer_count_commas_of_label(PPLexTable *lt) 
{
  int n;
  pp_label_node *p;
  if (lt->idx_of_active_label==-1) error(L"pp_lexer: current label is invalid");
  for (n=0,p=lt->nodes_of_label[lt->idx_of_active_label];p!=NULL;p=p->next)
    if (!wcscmp(p->str, L",")) n++;
  return n;
}

wchar_t **pp_lexer_get_next_group_of_tokens_of_label(PPLexTable *lt, int *n_tokens)
{ 
  /* all tokens until next comma, null-terminated */
  int n;
  pp_label_node *p;
  static wchar_t **tokens=0;  
  static int extents=0;

  p = lt->current_node_of_active_label;	   
  for (n = 0; p != NULL && wcscmp(p->str, L","); ++n, p = p->next);
  if (n > extents) {
     extents = n;
     free(tokens);
     tokens = (wchar_t **) malloc (extents * sizeof(wchar_t*));
  }   
...
static void set_label(PPLexTable *lt, const wchar_t *label) 
{
  int i;
  wchar_t *c;
  wchar_t *label_sans_colon;

  /* check for and then slice off the trailing colon */
  label_sans_colon = _wcsdup(label);
  c = &(label_sans_colon[wcslen(label_sans_colon) - 1]);
  if (*c != L':') error(L"Label %s must end with :", label);
  *c = 0;

  /* have we seen this label already? If so, abort */
  for (i = 0; lt->labels[i] != NULL && wcscmp(lt->labels[i], label_sans_colon); i++);
  if (lt->labels[i]!=NULL) 
    error(L"pp_lexer: label %s multiply defined!", label_sans_colon);

  /* new label. Store it */
  if (i == PP_LEXER_MAX_LABELS-1) 
    error(L"pp_lexer: too many labels. Raise PP_LEXER_MAX_LABELS");
...
static void add_string_to_label(PPLexTable *lt, wchar_t *str) 
{
  ...
  if (lt->idx_of_active_label==-1)
    error(L"pp_lexer: invalid syntax (line %i)",yylineno);
...
static void add_set_of_strings_to_label(PPLexTable *lt,const wchar_t *label_of_set) 
{
  ...
  if ((idx_of_label_of_set = get_index_of_label(lt, label_of_set))==-1)
    error(L"pp_lexer: label %s must be defined before it's referred to (line %i)"
...
static void show_bindings(PPLexTable *lt)
{
  ...
  wchar_t *la;
  pp_label_node *p;
  wprintf_s(L"The symbol table's contents: \n");
  for (i=0; (la=lt->labels[i])!=NULL; i++)
    {
      wprintf_s(L"\n\n%s\n", la);
      for (j=0; j<wcslen(la); j++)
	wprintf_s(L"=");
      wprintf_s(L"\n");
...
static int get_index_of_label(PPLexTable *lt, const wchar_t *label) 
{
  ...
  for (i=0; lt->labels[i]!=NULL; i++) 
    if (!wcscmp(lt->labels[i], label)) return i;
  return -1;
}
    
static void check_string(const wchar_t *str)
{
   if (wcslen(str)>1 && wcschr(str, L',')!=NULL)
      error(L"pp_lexer: string %s contains a comma, which is a no-no.",str);
}