...
void parse_options_set_display_constituents(Parse_Options opts, int dummy) {
    if ((dummy < 0) || (dummy > 2)) {
       fwprintf_s(stderr, L"Possible values for constituents: \n");
       fwprintf_s(stderr, L"   0 (no display) 1 (treebank style) or 2 (flat tree)\n");
 ...
 */
static Dictionary internal_dictionary_create(wchar_t * dict_name, wchar_t * pp_name, wchar_t * cons_name, wchar_t * affix_name, wchar_t * path) {
    Dictionary dict;
    static int rand_table_inited=FALSE;
    Dict_node *dict_node;
    wchar_t * dictionary_path_name;
...
int dictionary_delete(Dictionary dict) {
    ...
    if (verbosity > 0) {
	fwprintf_s(stderr, L"Freeing dictionary %s\n", dict->name);
    }
...
Sentence sentence_create(wchar_t *input_string, Dictionary dict) {
    ...
    sent->is_conjunction = (wchar_t *) xalloc(sizeof(wchar_t)*sent->length);
...
void sentence_delete(Sentence sent) {
  ...
  /*free_andlists(sent); */
    ...
    xfree(sent->is_conjunction, sizeof(wchar_t)*sent->length);
    xfree((wchar_t *) sent, sizeof(struct Sentence_s));
}
...
wchar_t * sentence_get_word(Sentence sent, int index) {
...
void post_process_linkages(Sentence sent, Parse_Options opts) {
    ...
    print_time(opts, L"Built parse set");
	...
	if (opts->verbosity > 1) 
	  fwprintf_s(stdout,
		  L"Warning: Count overflow.\n"
		  L"Considering a random subset of %d of an unknown and large number of linkages\n",
		  opts->linkage_limit);
    }
	...
	if (opts->verbosity > 1) 
	  fwprintf_s(stdout,
		  L"Warning: Considering a random subset of %d of %d linkages\n",
    ...
    print_time(opts, L"Postprocessed all linkages"); 
	...
	assert(! ((N_linkages_post_processed == 0) && 
		  ...
		  (N_linkages_found < opts->linkage_limit)),
	       L"None of the linkages is canonical");
    }

    if (opts->verbosity > 1) {
	fwprintf_s(stdout, L"%d of %d linkages with no P.P. violations\n", 
		N_valid_linkages, N_linkages_post_processed);
    }
	
    print_time(opts, L"Sorted all linkages");
...
void free_post_processing(Sentence sent) {
	...
	/* postprocessing must have been done */
        free_andlists(sent);
	xfree((wchar_t *) sent->link_info, sent->num_linkages_alloced*sizeof(Linkage_info));
	sent->link_info = NULL;
    }
}
...
int sentence_parse(Sentence sent, Parse_Options opts) {
    ...
    print_time(opts, L"Finished expression pruning");
	...
	print_time(opts, L"Counted parses");
    ...
    print_time(opts, L"Finished parse");
...
Linkage linkage_create(int k, Sentence sent, Parse_Options opts) {
    ...
    assert((k < sent->num_linkages_post_processed) && (k >= 0), L"index out of range");
    ...
    linkage->word = (wchar_t **) exalloc(linkage->num_words*sizeof(wchar_t *));
    linkage->current = 0;
...
void exfree_pp_info(PP_info ppi) {
	...
	exfree(ppi.domain_name[i], sizeof(wchar_t)*(wcslen(ppi.domain_name[i])+1));
    }
    if (ppi.num_domains > 0) 
	exfree(ppi.domain_name, sizeof(wchar_t *)*ppi.num_domains);
}

void xfree_pp_info(PP_info ppi) {
    int i;
    for (i=0; i<ppi.num_domains; ++i) {
	xfree(ppi.domain_name[i], sizeof(wchar_t)*(wcslen(ppi.domain_name[i])+1)); //added sizeof(wchar_t)*
    }
    xfree(ppi.domain_name, sizeof(wchar_t *)*ppi.num_domains);
}
...
void linkage_delete(Linkage linkage) {
	...
	exfree(linkage->word[i], sizeof(wchar_t)*(wcslen(linkage->word[i])+1));
    }
    exfree(linkage->word, sizeof(wchar_t *)*linkage->num_words);
	...
	if (s->violation != NULL) {
	    exfree(s->violation, sizeof(wchar_t)*(wcslen(s->violation)+1));
	}
    } 
...
int links_are_equal(Link l, Link m) {
    return ((l->l == m->l) && (l->r == m->r) && (wcscmp(l->name, m->name)==0));
}
...
PP_info excopy_pp_info(PP_info ppi) {
     ...
     newppi.domain_name = (wchar_t **) exalloc(sizeof(wchar_t *)*ppi.num_domains);
     for (i=0; i<newppi.num_domains; ++i) {
         newppi.domain_name[i] = (wchar_t *) exalloc(sizeof(wchar_t)*(wcslen(ppi.domain_name[i])+1));
	 wcscpy(newppi.domain_name[i], ppi.domain_name[i]);
     }
     return newppi;
}
...
Sublinkage unionize_linkage(Linkage linkage) {
	...
	for (j=0; j<linkage->sublinkage[i].num_links; ++j) {
		...
		if (((p=linkage->sublinkage[i].violation) != NULL) &&
		    (u.violation == NULL)) {
		    u.violation = (wchar_t *) exalloc((wcslen(p)+1)*sizeof(wchar_t));
		    wcscpy(u.violation, p);
		}
		num_in_union++;
	    }
	}
...
void linkage_post_process(Linkage linkage, Postprocessor * postprocessor) {
	...
	if (subl->violation != NULL) {
	    exfree(subl->violation, sizeof(wchar_t)*(wcslen(subl->violation)+1));
	    subl->violation = NULL;
	}
	...
	if (pp == NULL) {
		...
		if (k > 0) {
		    subl->pp_info[j].domain_name = (wchar_t **) exalloc(sizeof(wchar_t *)*k);
		}
		k = 0;
		for (d = pp->d_type_array[j]; d!=NULL; d=d->next) {
		    subl->pp_info[j].domain_name[k] = (wchar_t *) exalloc(sizeof(wchar_t)*2);
		    swprintf(subl->pp_info[j].domain_name[k], 1, L"%c", d->type);
		    k++;
		}
	    }
		...
		subl->violation = 
		    (wchar_t *) exalloc(sizeof(wchar_t)*(wcslen(pp->violation)+1));
		wcscpy(subl->violation, pp->violation);
	    }
	}
    }
